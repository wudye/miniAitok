...
public class WeatherQueryToolFunction implements Function<WeatherQueryToolFunction.Request, WeatherQueryToolFunction.Response> {
    ...
    private Response.WeatherInfo generateMockWeatherInfo() {
        int temperature = ThreadLocalRandom.current().nextInt(-5, 30);
        int humidity = ThreadLocalRandom.current().nextInt(1, 100);
        int windSpeed = ThreadLocalRandom.current().nextInt(1, 30);
        String condition = WEATHER_CONDITIONS[ThreadLocalRandom.current().nextInt(0, WEATHER_CONDITIONS.length)];
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
        return new Response.WeatherInfo(temperature, condition, humidity, windSpeed, LocalDateTime.now().format(formatter));
    }
