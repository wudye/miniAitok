// java
package com.mwu.aitokcommon.ai.model.xinghuo.api;

import com.fasterxml.jackson.annotation.JsonInclude;
import com.mwu.aitiokcoomon.core.utils.json.JsonUtils;
...
public class XunFeiPptApi {
    ...
    /**
     * 生成签名（替代 hutool SecureUtil + HmacAlgorithm）
     *
     * 签名生成逻辑：
     * 1. MD5(appId + timestamp) -> hex string
     * 2. HmacSHA1(apiSecret, auth) -> base64
     */
    private String generateSignature(String appId, String apiSecret, long timestamp) {
        try {
            // MD5(appId + timestamp) -> hex string
            MessageDigest md5 = MessageDigest.getInstance("MD5");
            byte[] md5Bytes = md5.digest((appId + timestamp).getBytes(StandardCharsets.UTF_8));
    ...
    public TemplatePageResponse getTemplatePage(String style, Integer pageSize) {
        ...
        requestBody.put("pageSize", ObjectUtils.defaultIfNull(pageSize, 20));
    ...
    public static <K, V> void addIfPresent(Map<K, V> map, K key, V value) {
        if (Objects.isNull(key) || Objects.isNull(map)) {
            return;
        }
